  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/5 bg-[#060814]/85 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between gap-6">
          ...
          {TABS.map((t) => {
            ...
            return (
              <Link
                key={t.href}
                href={t.href}
                className={`px-4 py-1.5 rounded-full text-[11px] uppercase tracking-wider whitespace-nowrap transition-colors ${
                  active
        ...
        <div className="h-8 w-8 rounded-full bg-white/[0.04] border border-white/5 flex items-center justify-center text-[10px] text-slate-400">
          NW
        </div>
      </div>
    </header>
